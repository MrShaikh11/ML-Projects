...
def classify_image(image_base64_data, file_path=None):
    ...
    # print("hel from classify img",imgs)
    result = []
    for img in imgs:
...
def get_cropped_image_if_2_eyes(image_path, image_base64_data):
    face_cascade = cv2.CascadeClassifier('M:\Coding\ML\Sportperson Image Classifier\server\opencv\haarcascades\haarcascade_frontalface_default.xml')
    eye_cascade = cv2.CascadeClassifier('M:\Coding\ML\Sportperson Image Classifier\server\opencv\haarcascades\haarcascade_eye.xml')

    if image_path:
        # print("hello from image path")
